...
class Insulter : CommandExecutor {
    ...
    private val generator = ShakespeareanInsultGenerator()

    @Command(
            aliases = [".insult"],
            showInHelpPage = true,
            usage = ".insult [@user ...]",
            description = "Insults one or more users Shakespeareanly."
    )
    fun insult(message: Message): String {
        ...
        return message.mentionedUsers.map { u -> u.name }.joinToString(", ") { n -> "**$n**" }
